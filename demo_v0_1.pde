PVector camera = new PVector(50,50,0);
PVector ray = new PVector(0,0,0);
float dist[] = new float[256];
float drawing,engle,distance,min_dist,max_dist,scl,bout;
float wid;
float sizeMap;
color skyCol,floorCol,blockCol,postCol;
void setup(){
  cursor(CROSS);
  surface.setResizable(true);
  SettingsMap();
  frameRate(100);
  size(640,360);
  fullScreen();
  drawing=100;
  ray.z=camera.z-engle/2;
  engle=1;
  min_dist=0.1;
  max_dist=500;
  scl=width/10;
  for(int y=0;y<map.length;y++){
    for(int x=0;x<map[0].length;x++){
      if(map[y][x]=='P'){
        camera.x=x*sizeMap;
        camera.y=y*sizeMap;
      }
    }
  }
}

void draw(){
  background(skyCol);
  fill(floorCol);
  noStroke();
  rect(0,height/2,width,height/2);
  float rot = mouseX;
  camera.z = rot/100;
  for(ray.z=camera.z-engle/2;ray.z<camera.z+engle/2;ray.z+=1/scl){
    ray.x=camera.x;
    ray.y=camera.y;
    distance=0;
    for(int i=0;i<drawing;i++){
      int m=0;
      for(int y=0;y<map.length;y++){
        for(int x=0;x<map[0].length;x++){
          if(map[y][x]=='N'){
            dist[m]=dBox(ray,new PVector(x*sizeMap,y*sizeMap),new PVector(sizeMap,sizeMap,0.7),blockCol);
          }
          else if(map[y][x]=='O'){
            dist[m]=dCircle(ray,new PVector(x*sizeMap,y*sizeMap),new PVector(sizeMap,0.7),postCol);
          }
          else if(map[y][x]=='n'){
            dist[m]=dBox(ray,new PVector(x*sizeMap,y*sizeMap),new PVector(sizeMap,sizeMap,0.5),blockCol);
          }
          else{
            dist[m]=max_dist+1;
          }
          m++;
        }
      }
      distance+=min(dist);
      if(min(dist)<min_dist){break;}
      ray.x+=min(dist)*cos(ray.z);
      ray.y+=min(dist)*sin(ray.z);
    }
    render();
  }
  fill(255);
  text(frameRate,0,10);
}
float dCircle(PVector r,PVector circle,PVector size,color col){
  float dist;
  dist=dist(r.x,r.y,circle.x,circle.y)-size.x/2;
  if(dist<min_dist){
    col=getLight(col);
    stroke(col);
    wid=size.y;
  }
  return dist;
}
float dBox(PVector r,PVector box,PVector size,color col){
  float dist;
  dist=dist(r.x,r.y,(max(abs(r.x-box.x)-size.x/2,0.0))+ray.x,(max(abs(r.y-box.y)-size.y/2,0.0))+ray.y);;
  if(dist<min_dist){
    col=getLight(col);
    stroke(col);
    wid=size.z;
  }
  return dist;
}

color getLight(color col){
  float black=distance/bout;
  col=color(red(col)-black,green(col)-black,blue(col)-black);
  return col;
}

void render(){
  float rayE;
  if(distance<max_dist){
    strokeWeight(width*1.3/scl);
    strokeCap(SQUARE);
    rayE=map(ray.z,-engle/2+camera.z,engle/2+camera.z,0,width);
    distance=width*20/distance;
    line(rayE,height/2-distance*wid,rayE,distance*0.5+height/2);
    stroke(50);
  }
}
